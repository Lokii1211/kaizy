...
export default function PerformancePage() {
  const [user, setUser] = useState<{
    id?: string; name?: string; kaizy_score?: number; trade?: string;
    worker_profile?: {
  ...
  useEffect(() => {
    fetch("/api/auth/me").then(r => r.json()).then(j => {
      if (j.success && j.data) {
        ...
        const trade = j.data.trade;

        // Fetch recent reviews for this worker
        if (userId) {
          fetch(`/api/reviews?workerId=${userId}&limit=5`)
            .then(r => r.json())
            .then(rj => {
              if (rj.success && rj.data?.length > 0) {
                setFeedback(rj.data.map((r: { name: string; area: string; rating: number; comment: string; created_at: string }) => {
                  const created = new Date(r.created_at);
                  const now = new Date();
                  const diffMs = now.getTime() - created.getTime();
                  const diffDays = Math.floor(diffMs / (1000 * 60 * 60 * 24));
                  const timeAgo = diffDays === 0 ? "Today" : diffDays === 1 ? "1 day ago" : diffDays < 7 ? `${diffDays} days ago` : diffDays < 30 ? `${Math.floor(diffDays / 7)} week${Math.floor(diffDays / 7) > 1 ? "s" : ""} ago` : `${Math.floor(diffDays / 30)} month${Math.floor(diffDays / 30) > 1 ? "s" : ""} ago`;
                  return {
        ...
        // Fetch pricing/market data as skill areas
        if (trade) {
          fetch(`/api/pricing/market?trade=${trade}`)
            .then(r => r.json())
            .then(pj => {
              if (pj.success && pj.data?.length > 0) {
                const totalJobs = j.data.worker_profile?.total_jobs || 0;
                setSkills(pj.data.slice(0, 5).map((p: { display_name: string; price_min: number; price_max: number }, idx: number) => {
                  // Distribute total jobs across services, weighted toward first items
                  const weight = 1 / (idx + 1);
                  const estimatedJobs = Math.round((weight / pj.data.slice(0, 5).reduce((_s: number, _: unknown, i: number) => _s + 1 / (i + 1), 0)) * totalJobs);
                  const level = estimatedJobs >= 50 ? "Expert" : estimatedJobs >= 20 ? "Advanced" : "Intermediate";
                  return {
                    name: p.display_name,
                    level,
                    jobs: estimatedJobs,
        ...
        fetch("/api/earnings?period=month")
          .then(r => r.json())
          .then(ej => {
            if (ej.success && ej.data?.length > 0) {
              const total = ej.data.reduce((sum: number, e: { amount?: number }) => sum + (e.amount || 0), 0);
  ...
  const ks = wp?.kaizy_score || user?.kaizy_score || 0;
  const rating = wp?.avg_rating || 0;
  const jobs = wp?.total_jobs || 0;
  const completionRate = wp?.completion_rate || (jobs > 0 ? Math.round((jobs / Math.max(jobs + 2, 1)) * 100) : 0);
      ...
      <div className="px-5 -mt-5 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2.5">
          {[
            ...
            { v: jobs > 0 ? `${jobs}` : "—", sub: "", l: "Total Jobs", icon: "📈" },
          ].map(s => (
        ...
        <div className="rounded-[18px] p-5" style={{ background: "var(--bg-card)", boxShadow: "var(--shadow-card)" }}>
          ...
          <p className="text-[32px] font-black mt-1" style={{ color: "var(--success)", fontFamily: "'JetBrains Mono', monospace" }}>
            {totalEarnings !== null ? (
              <>₹{totalEarnings.toLocaleString("en-IN")}<span className="text-[14px]">.00</span></>
            ) : "—"}
          </p>
