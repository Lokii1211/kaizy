...
export default function HirerDashboard() {
  ...
  useEffect(() => {
    ...
    // Fetch nearest worker ETA from real data
    fetch("/api/workers/nearby?limit=1").then(r => r.json()).then(j => {
      if (j.success && j.data?.workers?.length > 0) {
        setNearestEta(j.data.workers[0].eta);
      } else {
        setNearestEta(null);
      }
      ...
      <div className="px-5 pt-5 pb-3">
        <div className="flex justify-between items-center mb-1">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href="/notifications" aria-label="Notifications" className="w-9 h-9 rounded-xl flex items-center justify-center relative active:scale-90 transition-transform"
                  style={{ background: "var(--bg-surface)" }}>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[42px] font-black text-white leading-none mt-1"
                ...
                {nearestEta !== null ? nearestEta : "—"}<span className="text-[16px] font-bold text-white/60 ml-1">{nearestEta !== null ? "min" : ""}</span>
              </p>
