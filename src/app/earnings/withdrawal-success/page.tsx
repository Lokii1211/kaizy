...
interface ConfettiPiece { id: number; left: number; duration: number; delay: number; }
...
function WithdrawalContent() {
  ...
  // Generate random values only after mount — avoids SSR/client hydration mismatch
  useEffect(() => {
        ...
        <div className="fixed inset-0 pointer-events-none z-50 overflow-hidden">
          {confetti.map(p => (
            <div key={p.id}
                 ...
                 style={{
                   background: ['#FF6B00', '#34D399', '#3B82F6', '#F59E0B', '#EC4899'][p.id % 5],
                   left: `${p.left}%`,
                   top: `-5%`,
