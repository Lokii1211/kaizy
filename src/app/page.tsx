...
export default function HomePage() {
  ...
  // ── Voice search (mic button on home search bar) ──
  ...
  return (
    <div className="min-h-screen relative" style={{ background: "var(--bg-app)" }}>
      ...
      <div className="relative z-20" style={{ 
        ...
        <div className="flex justify-between items-center mb-3">
          ...
          <div className="flex items-center gap-2">
            {!userType && !authLoading && (
              <Link href="/login"
                    className="flex items-center gap-1.5 rounded-xl px-3.5 py-2 text-[12px] font-bold text-white active:scale-95 transition-transform"
            ...
            <button onClick={toggle} className="w-9 h-9 rounded-xl flex items-center justify-center active:scale-90 transition-transform"
                    style={{ background: "var(--bg-surface)" }} aria-label="Toggle theme">
            ...
            {userType && (
              <Link href="/notifications" className="w-9 h-9 rounded-xl flex items-center justify-center relative active:scale-90 transition-transform"
                    style={{ background: "var(--bg-surface)" }} aria-label="Notifications">
        ...
        {/* Value proposition for first-time visitors */}
        {!userType && !authLoading && (
          <div className="mb-4 text-center">
            <h1 className="text-[20px] font-black" style={{ color: "var(--text-1)", fontFamily: "'Epilogue', sans-serif" }}>
              Find verified workers in minutes
            </h1>
            <p className="text-[12px] mt-1 font-medium" style={{ color: "var(--text-3)" }}>
              Electricians, plumbers, mechanics & more — GPS-matched, background-checked, fairly priced.
            </p>
          </div>
        )}
        ...
        <div className="flex items-center gap-3 rounded-[16px] px-4 py-3.5"
             style={{
               ...
               border: isVoiceListening ? "2px solid var(--brand)" : "2px solid transparent",
               transition: "border 0.2s",
             }}>
          <Link href="/search" className="flex items-center gap-3 flex-1 min-w-0 active:scale-[0.98] transition-transform">
            <span className="text-[18px]">🔍</span>
            <span className="text-[13px] font-semibold flex-1 truncate" style={{ color: "var(--text-3)" }}>
          ...
          <button
            ...
            className="text-[14px] active:scale-90 transition-transform shrink-0"
            style={{
              color: isVoiceListening ? "var(--brand)" : "var(--brand)",
        ...
        {!userType && !authLoading && (
          <Link href="/login"
                className="flex items-center justify-center gap-2 mt-4 rounded-[14px] px-6 py-3.5 text-[14px] font-bold text-white active:scale-[0.97] transition-transform"
                style={{ background: "var(--brand)", boxShadow: "0 4px 14px rgba(255,107,0,0.3)" }}>
