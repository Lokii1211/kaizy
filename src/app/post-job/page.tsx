  ...
  const handlePostJob = async () => {
    ...
    try {
      const res = await fetch("/api/jobs/create", {
        ...
        body: JSON.stringify({
          trade: selectedTrade,
          problemType: description ? "custom" : "general",
          description: description || `${trades.find(t => t.id === selectedTrade)?.name} job`,
          lat: gpsLat || 11.0168,
          lng: gpsLng || 76.9558,
          address: location,
          isEmergency: false,
          ...
          jobType: mode,
          scheduledDate: mode === "scheduled" ? selectedDate : undefined,
          scheduledTime: mode === "scheduled" ? selectedTime : undefined,
          duration: mode === "project" ? duration : undefined,
        }),
      });
      const json = await res.json();
      if (json.success) {
        setJobId(json.data?.jobId || json.data?.id || `JOB-${Date.now()}`);
        setWorkersNotified(json.data?.workersNotified || 0);
        setIsPosted(true);
      } else {
        setPostError(json.error || "Failed to post job. Please try again.");
      }
    } catch {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6" style={{ background: "var(--bg-app)" }}>
        ...
        <div className="w-full rounded-[18px] p-5 mt-6" style={{ background: "var(--bg-card)" }}>
          <div className="space-y-3">
            {[
              ...
              ...(workersNotified > 0 ? [{ l: "Workers Notified", v: `${workersNotified}` }] : []),
            ].map(r => (
              <div key={r.l} className="flex justify-between">
                <span className="text-[10px] font-medium" style={{ color: "var(--text-3)" }}>{r.l}</span>
                <span className="text-[11px] font-bold" style={{ color: "var(--text-1)", fontFamily: r.l === "Budget" || r.l === "Job ID" ? "'JetBrains Mono', monospace" : "inherit" }}>{r.v}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="w-full mt-6 space-y-2.5">
          <button onClick={() => router.push(mode === "instant" ? "/booking" : "/marketplace")}
                  className="block w-full rounded-[16px] py-4 text-[13px] font-black text-white text-center active:scale-[0.97] transition-transform"
  ...
  return (
    <div className="min-h-screen pb-28" style={{ background: "var(--bg-app)" }}>
      ...
      <div className="px-5 pt-5 pb-3">
        <div className="flex items-center gap-3">
          <Link href="/" className="w-9 h-9 rounded-xl flex items-center justify-center active:scale-90 transition-transform"
                style={{ background: "var(--bg-surface)" }} aria-label="Go back">
      ...
      <div className="fixed bottom-0 left-0 right-0 px-5 py-4 z-50"
        ...
        {postError && (
          <div className="rounded-[12px] p-3 mb-2 flex items-center gap-2" style={{ background: "var(--danger-tint)" }}>
            <span className="text-[14px]">⚠️</span>
            <p className="text-[11px] font-bold flex-1" style={{ color: "var(--danger)" }}>{postError}</p>
            <button onClick={() => setPostError("")} className="text-[12px]" style={{ color: "var(--text-3)" }} aria-label="Close">✕</button>
          </div>
        )}
        <button onClick={handlePostJob} disabled={!selectedTrade || posting}
                className="w-full rounded-[16px] py-4 text-[13px] font-black text-white active:scale-[0.97] transition-transform disabled:opacity-40 flex items-center justify-center gap-2"
                style={{ background: "var(--gradient-cta)", boxShadow: "var(--shadow-brand)" }}>
          {posting ? (
            <>
