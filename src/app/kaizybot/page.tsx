...
// ============================================================
// KAIZYBOT — Real AI Chat via Claude API
// ============================================================
...
export default function KaizyBotPage() {
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      const res = await fetch("/api/kaizybot", {
        method: "POST",
    ...
    <div className="flex flex-col min-h-screen" style={{ background: "var(--bg-app)" }}>
      ...
      <div className="shrink-0 px-4 pt-4 pb-3" style={{ borderBottom: "1px solid var(--border-1)" }}>
        <div className="flex items-center gap-3">
          <Link href="/" aria-label="Go back" className="w-9 h-9 rounded-full flex items-center justify-center active:scale-90"
      ...
      <div className="shrink-0 px-4 py-3" style={{ borderTop: "1px solid var(--border-1)", background: "var(--bg-app)" }}>
        <div className="flex gap-2">
          <input ref={inputRef} value={input} onChange={e => setInput(e.target.value)}
                 ...
                 disabled={loading} />
          <button onClick={() => sendMessage(input)} disabled={!input.trim() || loading} aria-label="Send message"
