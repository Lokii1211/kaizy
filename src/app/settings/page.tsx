...
// ============================================================
// Kaizy — SETTINGS (Dark/Light Theme Toggle)
// ============================================================
...
const menuSections = [
  ...
  { title: "Support", items: [
    { icon: "💬", label: "KaamBot Help", href: "/konnectbot" },
  ...
  return (
    <div className="min-h-screen pb-24" style={{ background: "var(--bg-app)" }}>
      ...
      <div className="px-4 pt-4 pb-3">
        ...
        <div className="rounded-[16px] p-4 flex items-center gap-3" style={{ background: "var(--bg-card)", border: "1px solid var(--border-1)" }}>
          <div className="rounded-full flex items-center justify-center text-[22px] font-black text-white shrink-0"
               style={{ width: 56, height: 56, background: "var(--brand)", boxShadow: "var(--shadow-brand)" }}>R</div>
          <div className="flex-1">
            <p className="text-[15px] font-extrabold" style={{ color: "var(--text-1)" }}>Raju Kumar</p>
            <p className="text-[11px] font-bold" style={{ color: "var(--brand)" }}>⚡ Electrician · 10 yrs</p>
            <p className="text-[10px]" style={{ color: "var(--text-3)" }}>+91 98765 43210</p>
          </div>
      ...
      <div className="grid grid-cols-3 gap-2 px-4 mt-2">
        {[{ val: "4.9⭐", label: "Rating" }, { val: "312", label: "Jobs" }, { val: "742", label: "KaizyScore" }].map(s => (
      ...
      {/* ── THEME TOGGLE (prominent like Uber) ── */}
      ...
      <div className="px-4 mt-6">
        <Link href="/login" className="block w-full py-4 rounded-[14px] text-center text-[13px] font-extrabold active:scale-[0.98] transition-transform"
              style={{ background: "var(--bg-card)", color: "var(--danger)", border: "1px solid var(--danger-tint)" }}>Log Out</Link>
      </div>

      <p className="text-center text-[10px] mt-4 pb-4" style={{ color: "var(--text-3)", fontFamily: "'JetBrains Mono', monospace" }}>Kaizy v2.0.0 · By Antigravity</p>
    </div>
  );
}
