...
export async function POST(req: NextRequest) {
  try {
    ...
    const { workerId: bodyWorkerId, isOnline, latitude, longitude } = body;

    // Resolve workerId: prefer body, fallback to authenticated user
    let workerId = bodyWorkerId;
    if (!workerId) {
      const jwtPayload = await getUserFromRequest(req.cookies);
      if (jwtPayload) {
        workerId = jwtPayload.sub;
      }
    }

    if (!workerId) {
      return NextResponse.json({ success: false, error: 'workerId required or must be authenticated' }, { status: 400 });
    }
