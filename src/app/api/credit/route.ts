...
function calculateKaizyScore(workerData: Record<string, number>): {
  score: number;
  band: ScoreBand;
...
// POST — Calculate KaizyScore for a worker
export async function POST(request: NextRequest) {
  try {
    ...
    const result = calculateKaizyScore(worker_data);
...
export async function GET(request: NextRequest) {
  ...
  const result = calculateKaizyScore(workerData);
