...
// ============================================================
// KAIZY LEARN v10.0 — Stitch "Digital Artisan" Design
// Epilogue headlines · Tonal surfaces · Gradient CTAs
// Courses are content (hardcoded OK for MVP)
// Progress is now tracked via localStorage, no fake percentages
// ============================================================

const courses = [
  { id: 1, title: "House Wiring — Beginner to Expert", skill: "Electrician", icon: "⚡", lang: "Hindi + Tamil", duration: "12 hrs", lessons: 24, enrolled: "2.3K", rating: 4.8, badge: "Wiring Expert", free: true },
  { id: 2, title: "Solar Panel Installation", skill: "Electrician", icon: "☀️", lang: "Hindi", duration: "8 hrs", lessons: 16, enrolled: "1.5K", rating: 4.7, badge: "Solar Pro", free: true },
  { id: 3, title: "Modern Plumbing Techniques", skill: "Plumber", icon: "🔧", lang: "Hindi + Tamil", duration: "10 hrs", lessons: 20, enrolled: "1.8K", rating: 4.6, badge: "Adv Plumber", free: true },
  { id: 4, title: "Split AC Service & Repair", skill: "AC Tech", icon: "❄️", lang: "Hindi + English", duration: "14 hrs", lessons: 28, enrolled: "1.1K", rating: 4.9, badge: "AC Service Pro", free: false },
  { id: 5, title: "Furniture Making & Carpentry", skill: "Carpenter", icon: "🪚", lang: "Hindi", duration: "16 hrs", lessons: 32, enrolled: "980", rating: 4.5, badge: "Furniture Expert", free: true },
  { id: 6, title: "Industrial Welding — MIG & Arc", skill: "Welder", icon: "🔩", lang: "Hindi + Marathi", duration: "18 hrs", lessons: 36, enrolled: "670", rating: 4.7, badge: "Welder Pro", free: false },
];

function getStartedCourses(): Record<number, number> {
  if (typeof window === "undefined") return {};
  try {
    const raw = localStorage.getItem("kaizy_course_progress");
...
function markCourseStarted(courseId: number) {
  if (typeof window === "undefined") return;
  try {
    const progress = getStartedCourses();
    if (!progress[courseId]) {
      progress[courseId] = 1; // 1% = just started
    }
    localStorage.setItem("kaizy_course_progress", JSON.stringify(progress));
  } catch {
    // localStorage not available
  }
}

export default function KaizyLearnPage() {
  ...
  useEffect(() => {
    setCourseProgress(getStartedCourses());
  }, []);
  ...
  // Find courses the user has actually started
  const startedCourseIds = Object.keys(courseProgress).map(Number);
  const continueCourse = courses.find(c => startedCourseIds.includes(c.id));
  const continueProgress = continueCourse ? (courseProgress[continueCourse.id] || 0) : 0;
    ...
    <div className="min-h-screen pb-24" style={{ background: "var(--bg-app)" }}>
      ...
      <div className="pt-5 pb-7 px-5 rounded-b-[28px]" style={{ background: "var(--gradient-cta)" }}>
        <div className="flex items-center justify-between mb-5">
          ...
          <span className="text-[10px] font-bold text-white/60 uppercase tracking-widest">KaizyLearn</span>
      ...
      <div className="px-5 space-y-4 mt-5">
        ...
        {/* Continue Learning — only shown if user has actually started a course */}
        {continueCourse && continueProgress > 0 && (
          <div>
            ...
            <div className="rounded-[18px] p-4" style={{ background: "var(--bg-card)" }}>
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex-1">
                  <p className="text-[12px] font-bold" style={{ color: "var(--text-1)" }}>{continueCourse.title}</p>
                  <p className="text-[9px] font-medium" style={{ color: "var(--text-3)" }}>{continueCourse.lessons} lessons • {continueCourse.duration}</p>
                </div>
              </div>
              <div className="flex items-center gap-2 mb-3">
                <div className="flex-1 h-2 rounded-full overflow-hidden" style={{ background: "var(--bg-elevated)" }}>
                  <div className="h-full rounded-full" style={{ width: `${continueProgress}%`, background: "var(--gradient-cta)" }} />
                </div>
                <span className="text-[9px] font-bold" style={{ color: "var(--brand)", fontFamily: "'JetBrains Mono', monospace" }}>{continueProgress}%</span>
              </div>
        ...
        <div>
          ...
          <div className="space-y-2.5">
            {filtered.map(c => {
              const hasProgress = courseProgress[c.id] && courseProgress[c.id] > 0;
              return (
                <div key={c.id} className="rounded-[18px] overflow-hidden active:scale-[0.98] transition-all" style={{ background: "var(--bg-card)" }}>
                  <div className="p-4">
                    ...
                    <button onClick={() => handleStartCourse(c.id)}
                            className="w-full py-3 rounded-[14px] text-[11px] font-bold text-white flex items-center justify-center gap-2 active:scale-[0.97] transition-transform"
                            style={{ background: "var(--gradient-cta)", boxShadow: "var(--shadow-brand)" }}>
