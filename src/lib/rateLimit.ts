...
export const rateLimits = {
  ...
  jobCreate: (key: string) => checkRateLimit(`jobCreate:${key}`, 10, 60000),
  // Reviews: 5 per minute
  review: (key: string) => checkRateLimit(`review:${key}`, 5, 60000),
  // SOS: 3 per minute (prevent spam on emergency endpoint)
  sos: (key: string) => checkRateLimit(`sos:${key}`, 3, 60000),
