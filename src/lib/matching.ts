...
export function rankWorkers(workers: WorkerProfile[], query: ParsedQuery, hirerLat?: number, hirerLng?: number): MatchScore[] {
  return workers
    .map((worker) => {
      ...
      // KaizyScore bonus (0-10 points)
      breakdown.kaizyScoreBonus = Math.min(Math.round(worker.kaizyScore / 90), 10);
      if (worker.kaizyScore >= 750) reasons.push(`KaizyScore ${worker.kaizyScore}`);
